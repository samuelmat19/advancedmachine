	...
	var methods = {
		...
		get: function() {
			...
			if ( value && ( typeof moment != 'undefined' ) ) {
				...
				if ( cbdatepicker.settings.showTime && cbdatepicker.settings.timezone && ( typeof moment.tz != 'undefined' ) ) {
					date.tz( cbdatepicker.settings.timezone );
				}

				if ( date.isValid() ) {
					value = date.utc().format( cbdatepicker.settings.format );
				} else {
					value = '';
				}
			}
