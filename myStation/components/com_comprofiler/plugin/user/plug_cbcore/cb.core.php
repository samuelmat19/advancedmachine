...
class CBfield_text extends cbFieldHandler {
	...
	protected function getDataAttributes( $field, $user, $output, $reason, $attributeArray = array() ) {
		...
		if ( $forbiddenContent != '' ) {
			...
			$attributeArray[]		=	cbValidator::getRuleHtmlAttributes( 'forbiddenwords', $forbiddenContent, CBTxt::T( 'UE_INPUT_VALUE_NOT_ALLOWED', 'This input value is not authorized.' ) );
		}
...
class CBfield_predefined extends CBfield_text {
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types ) {
		...
		switch ( $output ) {
			...
			case 'rss':
				...
				} elseif ( ( $field->type == 'predefined' ) && $user->get( '_allowProfileLink', $field->get( '_allowProfileLink', 1 ) ) && ( $reason != 'profile' ) && ( $reason != 'edit' ) ) {
	...
	public function fieldClass( &$field, &$user, &$postdata, $reason ) {
		...
		if ( ( $function == 'checkvalue' ) || ( $function == 'testexists' ) ) {
			...
			if ( $ajaxChecker && ( $value != '' ) && ( ( $reason == 'edit' ) || ( $reason == 'register' ) ) ) {
		...
		switch ( $output ) {
			...
			case 'rss':
				if ( $value == null ) {
					...
					return $this->formatFieldValueLayout( '<a href="' . htmlspecialchars( ( ! $scheme ? 'http://' : null ) . $oReturn[0] ) . '" target="_blank"' . ( (int) $field->params->get( 'webaddress_nofollow', 1 ) ? ' rel="nofollow"' : null ) . '>' . htmlspecialchars( $oReturn[1] ) . '</a>', $reason, $field, $user );
				} else {
