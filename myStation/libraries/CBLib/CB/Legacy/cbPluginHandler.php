	...
	public function getPluginVersion( $plugin, $raw = false, $duration = 24, $length = 0 )
	{
			...
			if ( Application::Config()->get( 'pluginVersionCheck', true, GetterInterface::BOOLEAN ) ) {
				...
				if ( $request ) {
					try {
						...
						$results						=	$guzzleHttpClient->get( 'http://update.joomlapolis.net/cbpluginsversions20.xml', array( 'timeout' => 10 ) );
		...
		if ( $latestCoreVersion === null ) {
			...
			if ( $request ) {
				try {
					$guzzleHttpClient					=	new GuzzleHttp\Client();
					$guzzleRequest						=	$guzzleHttpClient->get( 'http://www.joomlapolis.com/versions/comprofilerversion.php?currentversion=' . urlencode( CBLib::versionWithBuild() ), array( 'headers' => array( 'referer' =>  $_CB_framework->getCfg( 'live_site' ) ), 'timeout' => 10 ) );
